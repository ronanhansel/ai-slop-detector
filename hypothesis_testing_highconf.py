...
def hypothesis_c(df, influencer_leaning=None):
    ...
    df_merged = df_merged[df_merged['leaning'].isin(['Left', 'Right', 'Neutral'])]
    
    if len(df_merged) < 100:
        results['error'] = f'Insufficient data after merge ({len(df_merged)} rows)'
        return results
    
    print(f"Merged {len(df_merged)} comments with political leaning data")
    ...
    for leaning in ['Left', 'Right', 'Neutral']:
        for label in [0, 1]:
            ...
            groups_data[key] = subset
    
    # One-Way ANOVA across all groups
    valid_groups = [g for g in groups_data.values() if len(g) > 0]
    if len(valid_groups) < 2:
        results['error'] = 'Insufficient groups for ANOVA'
        return results
    
    f_stat, p_val = f_oneway(*valid_groups)
    
    # Store group statistics
    results['groups'] = {k: {'mean': v.mean(), 'n': len(v), 'std': v.std()} 
                        for k, v in groups_data.items() if len(v) > 0}
    results['anova'] = {'f_stat': f_stat, 'p_value': p_val, 'significant': p_val < 0.05}
    results['conclusion'] = 'INTERACTION FOUND' if p_val < 0.05 else 'NO SIGNIFICANT INTERACTION'
...
def create_visualizations(df, results_a, results_b, results_c, results_d):
    ...
    axes[0,0].boxplot([humans, bots], labels=['Human', 'Bot'])
...
def hypothesis_a_highconf(df):
    """Hypothesis A on high-confidence data only (ai_confidence > 0.9 or < 0.1)."""
    results = {'hypothesis': 'A (High Confidence)', 'title': 'AI Slop vs Human Rage-Bait - High Confidence Data'}
    
    # Filter for high confidence: >0.9 or <0.1
    df_hc = df[(df['ai_confidence'] > 0.9) | (df['ai_confidence'] < 0.1)].copy()
    
    if len(df_hc) < 50:
        results['error'] = f'Insufficient high-confidence data ({len(df_hc)} rows)'
        return results
    
    print(f"High-confidence subset: {len(df_hc)} comments ({100*len(df_hc)/len(df):.1f}% of total)")
    
    bots = df_hc[df_hc['label'] == 1]['RBI']
    humans = df_hc[df_hc['label'] == 0]['RBI']
    
    if len(bots) > 5 and len(humans) > 5:
        u_stat, p_val = mannwhitneyu(bots, humans, alternative='greater')
        results['test'] = {
            'n_bots': len(bots), 'n_humans': len(humans),
            'bot_mean': bots.mean(), 'human_mean': humans.mean(),
        ...
        results['error'] = f'Insufficient samples (bots={len(bots)}, humans={len(humans)})'
    
    return results

def hypothesis_b_highconf(df):
    """Hypothesis B on high-confidence data only."""
    results = {'hypothesis': 'B (High Confidence)', 'title': 'Trigger Effect - High Confidence Data'}
    ...
    # Get root comments from high-confidence data
    roots = df_hc[df_hc['is_root']].copy()
    
    # For each root, calculate EV of replies
    root_ev = []
    for _, root in roots.iterrows():
        replies = df_hc[df_hc['parent_id'] == root['comment_id']]
        if len(replies) >= 2:
            ev = replies['sentiment_prob'].var() + replies['offensive_prob'].var()
            root_ev.append({
                'comment_id': root['comment_id'],
    ...
    if not root_ev:
        results['error'] = 'Insufficient reply data in high-confidence subset'
        return results
...
def hypothesis_c_highconf(df, influencer_leaning=None):
    """Hypothesis C on high-confidence data only."""
    results = {'hypothesis': 'C (High Confidence)', 'title': 'Side-Specific Polarization - High Confidence Data'}
    ...
    # Merge chain: posts -> leaning -> comments
    posts['author_id'] = posts['author_id'].astype(str)
    influencer_leaning['author_id'] = influencer_leaning['author_id'].astype(str)
    posts_leaning = posts.merge(influencer_leaning, on='author_id', how='inner')
    
    if len(posts_leaning) == 0:
        results['error'] = 'No matching authors'
        return results
    
    df_hc['post_id'] = df_hc['post_id'].astype(str)
    posts_leaning['post_id'] = posts_leaning['post_id'].astype(str)
    df_merged = df_hc.merge(posts_leaning[['post_id', 'leaning']], 
                            left_on='post_id', right_on='post_id', how='inner')
    df_merged = df_merged[df_merged['leaning'].isin(['Left', 'Right', 'Neutral'])]
    
    if len(df_merged) < 50:
        results['error'] = f'Insufficient merged data ({len(df_merged)} rows)'
        return results
...
def hypothesis_d_highconf(df, top_pct=0.01):
    """Hypothesis D on high-confidence data only."""
    results = {'hypothesis': 'D (High Confidence)', 'title': 'Super-Spreaders - High Confidence Data'}
    ...
    if df_hc['label'].dtype == 'object':
        df_hc['label'] = pd.to_numeric(df_hc['label'], errors='coerce')
    
    user_stats = df_hc.dropna(subset=['label']).groupby('commenter_id').agg({
        'RBI': 'mean',
        'label': 'mean',
        'ai_confidence': 'mean',
        'comment_id': 'count'
    ...
    pct_synthetic = prob_bot_count / n_top * 100
    hard_bot_count = (top_users['label'] >= 0.5).sum()
    baseline_pct = df_hc['label'].mean() * 100
...
def generate_report_highconf(results_a_hc, results_b_hc, results_c_hc, results_d_hc, df):
    """Generate high-confidence subset report."""
    
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    df_hc = df[(df['ai_confidence'] > 0.9) | (df['ai_confidence'] < 0.1)]
    
    report = f"""
================================================================================
        AI SLOP DETECTION - HIGH CONFIDENCE ANALYSIS (ai_confidence > 0.9 or < 0.1)
                              Generated: {timestamp}
================================================================================

HIGH-CONFIDENCE SUBSET SUMMARY
------------------------------
Total Comments in Subset: {len(df_hc):,} ({100*len(df_hc)/len(df):.1f}% of total)
Bot Comments: {(df_hc['label']==1).sum():,} ({100*(df_hc['label']==1).mean():.1f}%)
Human Comments: {(df_hc['label']==0).sum():,} ({100*(df_hc['label']==0).mean():.1f}%)
Mean RBI: {df_hc['RBI'].mean():.4f} (std: {df_hc['RBI'].std():.4f})

================================================================================
HYPOTHESIS A (High Confidence): {results_a_hc['title']}
================================================================================
"""
    if 'test' in results_a_hc:
        report += f"""Results:
- Bot comments → Mean RBI: {results_a_hc['test']['bot_mean']:.4f}, n={results_a_hc['test']['n_bots']}
- Human comments → Mean RBI: {results_a_hc['test']['human_mean']:.4f}, n={results_a_hc['test']['n_humans']}
- Mann-Whitney U: {results_a_hc['test']['u_stat']:.2f}, p-value: {results_a_hc['test']['p_value']:.6f}

CONCLUSION: {results_a_hc['conclusion']}
"""
    else:
        report += f"ERROR: {results_a_hc.get('error', 'Unknown error')}\n"

    report += f"""
================================================================================
HYPOTHESIS B (High Confidence): {results_b_hc['title']}
================================================================================
"""
    if 'test' in results_b_hc:
        report += f"""Results:
- Bot root comments → Mean reply EV: {results_b_hc['test']['bot_mean_ev']:.4f}
- Human root comments → Mean reply EV: {results_b_hc['test']['human_mean_ev']:.4f}
- Mann-Whitney U: {results_b_hc['test']['u_stat']:.2f}, p-value: {results_b_hc['test']['p_value']:.6f}

CONCLUSION: {results_b_hc['conclusion']}
"""
    else:
        report += f"ERROR: {results_b_hc.get('error', 'Unknown error')}\n"

    report += f"""
================================================================================
HYPOTHESIS C (High Confidence): {results_c_hc['title']}
================================================================================
"""
    if results_c_hc and 'anova' in results_c_hc:
        report += f"""Group Statistics (RBI by Political Lean × Bot/Human):
"""
        groups_df = pd.DataFrame([{'group': k, **v} for k, v in results_c_hc['groups'].items()])
        report += groups_df.to_string(index=False)
        report += f"""

ANOVA Results:
- F-statistic: {results_c_hc['anova']['f_stat']:.4f}
- p-value: {results_c_hc['anova']['p_value']:.6f}
- Significant: {'Yes' if results_c_hc['anova']['significant'] else 'No'} (α=0.05)

CONCLUSION: {results_c_hc['conclusion']}
"""
    else:
        report += f"ERROR: {results_c_hc.get('error', 'Unknown error')}\n"

    report += f"""
================================================================================
HYPOTHESIS D (High Confidence): {results_d_hc['title']}
================================================================================
Results:
- Analyzed top {results_d_hc['stats']['n_top_users']} users by average RBI
- Probabilistic bot count: {results_d_hc['stats']['prob_bot_count']:.1f}
- Estimated % synthetic: {results_d_hc['stats']['pct_synthetic']:.1f}%
- Baseline bot %: {results_d_hc['stats']['baseline_bot_pct']:.1f}%
- Enrichment factor: {results_d_hc['stats']['enrichment']:.2f}x

CONCLUSION: {results_d_hc['conclusion']}
...
def correlation_analysis_highconf(df):
    """Analyze correlations on high-confidence data only."""
    results = {'hypothesis': 'CORRELATION (High Confidence)', 'title': 'Feature-Confidence Correlations - High Confidence Data'}
    ...
    print(f"Correlation analysis on {len(df_hc)} high-confidence samples")
    
    # Select features for correlation
    features = ['s', 'h', 'o', 'i', 'r', 'RBI', 'ai_confidence']
    corr_data = df_hc[features].copy()
    ...
    p_values = {}
    for col in ['s', 'h', 'o', 'i', 'r', 'RBI']:
        valid = df_hc[[col, 'ai_confidence']].dropna()
        if len(valid) > 2:
            _, p_val = pearsonr(valid[col], valid['ai_confidence'])
            p_values[col] = p_val
    ...
    results['strongest'] = ai_pearson.idxmax()
    results['strongest_r'] = ai_pearson.max()
    results['n_samples'] = len(df_hc)
    
    return results

def plot_correlations_highconf(df, corr_results_hc):
    """Visualize correlation matrix for high-confidence data."""
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    ...
    # Heatmap of all correlations
    features = ['s', 'h', 'o', 'i', 'r', 'RBI', 'ai_confidence']
    corr_matrix = df_hc[features].corr(method='pearson')
    
    sns.heatmap(corr_matrix, annot=True, fmt='.3f', cmap='coolwarm', center=0, 
                ax=axes[0], cbar_kws={'label': 'Correlation'})
    axes[0].set_title('Feature Correlation Matrix (High Confidence)')
    
    # Bar plot: Feature importance for AI confidence
    importance = corr_results_hc['abs_magnitude'].sort_values(ascending=True)
    colors = ['green' if corr_results_hc['pearson'][feat] > 0 else 'red' for feat in importance.index]
    axes[1].barh(range(len(importance)), importance.values, color=colors, alpha=0.7)
    axes[1].set_yticks(range(len(importance)))
    axes[1].set_yticklabels(importance.index)
    axes[1].set_xlabel('|Pearson Correlation| with AI Confidence')
    axes[1].set_title('Feature Importance (High Confidence)')
    axes[1].grid(alpha=0.3, axis='x')
    
    plt.tight_layout()
    plt.savefig('data-ai-slop-detector/correlation_analysis_highconf.png', dpi=150, bbox_inches='tight')
    plt.close()

def report_correlations_highconf(corr_results_hc):
    """Generate correlation report for high-confidence data."""
    report = f"""
================================================================================
CORRELATION ANALYSIS: Feature-Confidence Relationships (High Confidence)
================================================================================
Question: Which emotional features best predict AI confidence in high-confidence data?
Sample size: {corr_results_hc.get('n_samples', 'N/A')} comments

Pearson Correlations with AI Confidence (ranked by strength):
"""
    for feat, val in corr_results_hc['pearson'].items():
        p_val = corr_results_hc['p_values'].get(feat, np.nan)
        sig = '***' if p_val < 0.001 else '**' if p_val < 0.01 else '*' if p_val < 0.05 else 'ns'
        report += f"  {feat:8s}: r = {val:7.4f}  p = {p_val:.2e}  {sig}\n"
...
Spearman Correlations (rank-based, robust to outliers):
"""
    for feat, val in corr_results_hc['spearman'].items():
        report += f"  {feat:8s}: rho = {val:7.4f}\n"
    
    report += f"""
STRONGEST PREDICTOR: {corr_results_hc['strongest']} (r = {corr_results_hc['strongest_r']:.4f})
...
def main():
    ...
    print("\n" + "="*80)
    print("HIGH-CONFIDENCE SUBSET ANALYSIS (ai_confidence > 0.9 or < 0.1)")
    print("="*80)
    
    print("Testing Hypothesis A (High Confidence)...")
    results_a_hc = hypothesis_a_highconf(df)
    
    print("Testing Hypothesis B (High Confidence)...")
    results_b_hc = hypothesis_b_highconf(df)
    
    print("Testing Hypothesis C (High Confidence)...")
    results_c_hc = hypothesis_c_highconf(df)
    
    print("Testing Hypothesis D (High Confidence)...")
    results_d_hc = hypothesis_d_highconf(df)
    
    print("Correlation Analysis (High Confidence)...")
    corr_results_hc = correlation_analysis_highconf(df)
    
    print("\nGenerating reports...")
    report = generate_report(results_a, results_b, results_c, results_d, df)
    report += report_correlations(corr_results)
    
    report_hc = generate_report_highconf(results_a_hc, results_b_hc, results_c_hc, results_d_hc, df)
    report_hc += report_correlations_highconf(corr_results_hc)
    
    # Save reports
    with open('data-ai-slop-detector/hypothesis_testing_report.txt', 'w', encoding='utf-8') as f:
        f.write(report)
    
    with open('data-ai-slop-detector/hypothesis_testing_report_highconf.txt', 'w', encoding='utf-8') as f:
        f.write(report_hc)
    ...
    plot_correlations_highconf(df, corr_results_hc)
    
    print("\nSaved:")
    print("  - data-ai-slop-detector/hypothesis_testing_report.txt")
    print("  - data-ai-slop-detector/hypothesis_testing_report_highconf.txt")
    print("  - data-ai-slop-detector/hypothesis_testing_results.png")
    print("  - data-ai-slop-detector/correlation_analysis.png")
    print("  - data-ai-slop-detector/correlation_analysis_highconf.png")
    
    return df, results_a, results_b, results_c, results_d, corr_results, results_a_hc, results_b_hc, results_c_hc, results_d_hc, corr_results_hc
